...
def worker(bits):
    ...
    if bits[0] == 'do_update':
        func, model, start, end, total, optional_site, age, verbosity2 = bits
    elif bits[0] == 'do_remove':
        func, model, pks_seen, start, upper_bound, optional_site, verbosity2 = bits
    else:
        return
    ...
    if func == 'do_update':
        qs = build_queryset(index, model, age=age, verbosity2=verbosity2)
        do_update(index, qs, start, end, total, verbosity2=verbosity2)
    elif bits[0] == 'do_remove':
        do_remove(index, model, pks_seen, start, upper_bound, verbosity2=verbosity2)
...
def build_queryset(index, model, age=DEFAULT_AGE, verbosity2=1):
    extra_lookup_kwargs = {}
...
def do_update(index, qs, start, end, total, verbosity2=1):
...
def do_remove(index, model, pks_seen, start, upper_bound, verbosity2=1):
...
class Command(AppCommand):
    ...
    verbosity2_present = False

    for option in option_list:
        if option.get_opt_string() == '--verbosity2':
            verbosity2_present = True

    if verbosity2_present is False:
        option_list = option_list + (
            make_option('--verbosity2', action='store', dest='verbosity2', default='1',
                type='choice', choices=['0', '1', '2'],
                help='verbosity2 level; 0=minimal output, 1=normal output, 2=all output'
            ),
        )

    def handle(self, *apps, **options):
        self.verbosity2 = int(options.get('verbosity2', 1))
    ...
    def handle_app(self, app, **options):
        ...
        for model in get_models(app):
            try:
                ...
                if self.verbosity2 >= 2:
                    print "Skipping '%s' - no index." % model
                continue

            qs = build_queryset(index, model, age=self.age, verbosity2=self.verbosity2)
            total = qs.count()

            if self.verbosity2 >= 1:
            ...
            for start in range(0, total, self.batchsize):
                ...
                if self.workers == 0:
                    do_update(index, qs, start, end, total, self.verbosity2)
                else:
                    ghetto_queue.append(('do_update', model, start, end, total, self.site, self.age, self.verbosity2))
            ...
            if self.remove:
                ...
                for start in range(0, total, self.batchsize):
                    ...
                    if self.workers == 0:
                        ...
                        ghetto_queue.append(('do_remove', model, pks_seen, start, upper_bound, self.site, self.verbosity2))
